...
-(NSArray *)operateTitleArray{
    return [NSArray arrayWithObjects:@"位移",@"透明度",@"缩放",@"旋转",@"背景色"
           //, @"margin",@"zPosition"
            , nil];
}
...
-(void)rotateAnimation{
    CABasicAnimation *anima = [CABasicAnimation animationWithKeyPath:@"transform.rotation.y"];//绕着z轴为矢量，进行旋转(@"transform.rotation.z"==@@"transform.rotation")
...
- (void)marginAnimation {
    CABasicAnimation *anima = [CABasicAnimation animationWithKeyPath:@"margin"];
    anima.toValue = [NSNumber numberWithFloat:1.0];
    anima.duration = 1.0f;
    [_demoView.layer addAnimation:anima forKey:@"marginAnimation"];
}
